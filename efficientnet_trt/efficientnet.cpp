...
#define NUM_OF_CLASSES 3 // By default this was 1000.
...
static std::map<std::string, GlobalParams>
	global_params_map = {
		...
		{"b0", GlobalParams{224, 224, NUM_OF_CLASSES, 0.001, 1.0, 1.0, 8, -1}},
		{"b1", GlobalParams{240, 240, NUM_OF_CLASSES, 0.001, 1.0, 1.1, 8, -1}},
		{"b2", GlobalParams{260, 260, NUM_OF_CLASSES, 0.001, 1.1, 1.2, 8, -1}},
		{"b3", GlobalParams{300, 300, NUM_OF_CLASSES, 0.001, 1.2, 1.4, 8, -1}},
		{"b4", GlobalParams{380, 380, NUM_OF_CLASSES, 0.001, 1.4, 1.8, 8, -1}},
		{"b5", GlobalParams{456, 456, NUM_OF_CLASSES, 0.001, 1.6, 2.2, 8, -1}},
		{"b6", GlobalParams{528, 528, NUM_OF_CLASSES, 0.001, 1.8, 2.6, 8, -1}},
		{"b7", GlobalParams{600, 600, NUM_OF_CLASSES, 0.001, 2.0, 3.1, 8, -1}},
		{"b8", GlobalParams{672, 672, NUM_OF_CLASSES, 0.001, 2.2, 3.6, 8, -1}},
		{"l2", GlobalParams{800, 800, NUM_OF_CLASSES, 0.001, 4.3, 5.3, 8, -1}},
};

void normalize(cv::Mat &img, const std::vector<float> &mean, const std::vector<float> &std)
{
	// Normalize the image given the mean and std.
	assert(img.channels() == mean.size() && img.channels() == std.size());
	for (int c = 0; c < img.channels(); ++c)
	{
		img.forEach<cv::Vec3f>([c, &mean, &std](cv::Vec3f &pixel, const int *position) -> void
							   { pixel[c] = (pixel[c] - mean[c]) / std[c]; });
	}
}

void preprocessImage(const std::string &image_path, float *input, int input_h, int input_w)
{
	...
	normalize(img, mean, std); // TODO remove once you fix python side.
	...
	for (int row = 0; row < img.rows; ++row)
	{
		for (int col = 0; col < img.cols; ++col)
		{
			...
			{
				input[i] = img.at<cv::Vec3f>(row, col)[ch];
				++i;
			}
		}
	}
...
bool parse_args(int argc, char **argv, std::string &wts, std::string &engine, std::string &backbone)
{
	...
	// {
	// 	wts = "/home/utku/dfas_classifier/trained_models/exp_light_dataset_v1_other_decimated_classifier_effnet_1_layer_freeze/light_dataset_v1_other_decimated_classifier_effnet_f1_corrected.wts";
	// 	engine = "temp.engine";
	// 	backbone = "b0";
...
int main(int argc, char **argv)
{
	...
	// Real input
	std::string image_path = "green.png";
	float *data = new float[3 * global_params.input_h * global_params.input_w];
	preprocessImage(image_path, data, global_params.input_h, global_params.input_w);
